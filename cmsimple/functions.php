...
{
	global $adm, $edit, $s, $u, $sn, $tx, $sl, $cf, $su, $pth, $disabled_plugins, $active_plugins; // pluginmanager (added "$pth, $disabled_plugins")
...
// END pluginmanager (changed)
		...
		$t .= '<li><a href="' . $sn . '?&amp;normal&amp;xhpages" class="">' . ucfirst($tx['adminmenu']['pagemanager']) . '</a></li>' . "\n";
		$t .= '<li><a href="' . $sn . '?&amp;normal&amp;userfiles">' . ucfirst($tx['adminmenu']['files']) . '</a>' ."\n";
		$t .= '    <ul>' . "\n";
		$t .= '    <li><a href="' . $sn . '?&amp;normal&amp;images">' . ucfirst($tx['adminmenu']['images']) . '</a></li>' . "\n";
		$t .= '    <li><a href="' . $sn . '?&amp;normal&amp;downloads">' . ucfirst($tx['adminmenu']['downloads']) . '</a></li>' . "\n";
		$t .= '    <li><a href="' . $sn . '?&amp;normal&amp;media">' . ucfirst($tx['adminmenu']['media']) . '</a></li>' . "\n";
		$t .= '    </ul>' . "\n";
		$t .= '</li>' ."\n";
...
function languagemenu() 
{
	...
	{ 
		...
		{
			$t .= '<a href="'.$pth['folder']['base'].'">'.tag('img src="'.$pth['folder']['flags'].$cf['language']['default'].'.gif" alt="'.$cf['language']['default'].'" title="&nbsp;'.$cf['language']['default'].'&nbsp;" class="flag"').'</a> '; 
		}
		$v = count($r); for($i = 0;
		$i < $v;
		$i++) 
	...
	{ 
		...
		if($cf['language']['default'] != $sl)
		{
			$t='<a href="../">'.tag('img src="'.$pth['folder']['flags'].$cf['language']['default'].'.gif" alt="'.$cf['language']['default'].'" title="&nbsp;'.$cf['language']['default'].'&nbsp;" class="flag"').'</a> ';
		}
		else
		{
			$t .= ''; 
		...
		$i++) 
		{
			if ($sl != $r[$i]) 
			{
				if (is_file($pth['folder']['flags'].'/'.$r[$i].'.gif')) 
				{
					$t .= '<a href="../'.$r[$i].'/">'.tag('img src="'.$pth['folder']['flags'].$r[$i].'.gif" alt="'.$r[$i].'" title="&nbsp;'.$r[$i].'&nbsp;" class="flag"').'</a> ';
				} 
				else 
				{
					$t .= '<a href="../'.$r[$i].'/">['.$r[$i].']</a> ';
