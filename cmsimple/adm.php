...
function selectlist($fn, $regm, $regr) 
{
    ...
    {
		if($option != '__fallback__' && $option != '__maintenance__' && $option != '__cmsimple_backend__') // template fallback (hide standard templates)
		{
...
// System Info, Version Check and Help Links
	...
	if ($f == 'sysinfo') 
	{
		...
		foreach($pluginlist_array as $pluginlist_item)
		{
			...
			$cf['site']['allow_versionsinfo'] == 'true')
			{
				$cmsimple_pluginversion_user = explode(',', file_get_contents($pth['folder']['plugins'] . $pluginlist_item . '/version.nfo'));
...
// single line input field or textarea depending on text length
...
function cmsimple_pluginmanager() 
{
	...
	{
		$pm_output.='<p class="cmsimplecore_warning" style="font-weight: 300; text-align: center;">' . $pluginloader_tx['pluginmanager']['message_datafile1'] . ' <b>disabled_plugins.txt</b> ' . $pluginloader_tx['pluginmanager']['message_datafile2'] . '</p>';
	}
